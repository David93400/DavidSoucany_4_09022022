...
const validateField = (regex, field, fieldError, long=0) => {
  ...
  if (regex.test(field) && field.length >= long) {
...
firstName.addEventListener("focusout", () => validateField(regexName, firstName, firstNameError, 2))
lastName.addEventListener("focusout", () => validateField(regexName, lastName, lastNameError, 2))
email.addEventListener("focusout", () => validateField(regexMail, email, mailError))
birthday.addEventListener("focusout", () => validateField(regexDate, birthday, birthdayError))
quantityCity.addEventListener("focusout", () => validateField(regexNumber, quantityCity, quantityCityError))
